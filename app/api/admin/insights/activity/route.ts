...
export async function GET(req: NextRequest) {
  ...
  const items = await listUserActivity(user._id.toString(), opts);
  if (format === "csv") {
    const header = "activityId,createdAt,type,title,threadId\n";
    const rows = items.map((a: any) => `${a._id || ""},${new Date(a.createdAt).toISOString()},${a.type || ""},${(a.title || "").replace(/,/g, " ")},${a.threadId || ""}`).join("\n");
