    ...
    <div className="flex h-screen overflow-hidden relative bg-white">
      ...
      <main className="flex-1 h-full overflow-y-auto relative z-10">
        <div className="h-full w-full px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-8 pb-6 border-b border-neutral-200">
            <div className="flex items-center gap-4">
              <div className="relative">
                <div className="flex items-center justify-center h-14 w-14 rounded-2xl bg-gradient-to-br from-emerald-400 to-emerald-600 shadow-lg">
                  <HiOutlinePlus className="h-7 w-7 text-white" />
                </div>
              </div>
              <div>
                <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-emerald-800 tracking-tight">
                  My Ingredients
                </h1>
            ...
            <button
              onClick={() => openModal()}
              className="flex items-center gap-2 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-bold rounded-xl px-6 py-3 shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5"
            >
          ...
          <div className="mb-6">
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full bg-white border-2 border-neutral-200 rounded-xl pl-12 pr-4 py-3 text-sm text-black placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:border-emerald-300 transition"
              />
            </div>
          </div>
          ...
          {!loading && filteredIngredients.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 bg-gradient-to-br from-neutral-50 to-white rounded-3xl border-2 border-dashed border-neutral-300">
              <div className="h-20 w-20 rounded-full bg-gradient-to-br from-emerald-100 to-emerald-200 flex items-center justify-center mb-4">
                <HiOutlinePlus className="h-10 w-10 text-emerald-600" />
              </div>
              <h3 className="text-xl font-bold text-neutral-800 mb-2">No ingredients yet</h3>
              <p className="text-sm text-neutral-600 mb-6">Start building your pantry by adding ingredients!</p>
              <button
                onClick={() => openModal()}
                className="flex items-center gap-2 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-bold rounded-xl px-6 py-3 shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5"
              >
                <HiOutlinePlus className="w-5 h-5" />
          ...
          {!loading && filteredIngredients.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredIngredients.map((ing) => (
                <div
                  key={ing._id}
                  className="bg-neutral-50/90 rounded-2xl shadow-lg border-2 border-neutral-300 p-5 hover:shadow-xl hover:border-emerald-300 transition-all group"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-neutral-900 group-hover:text-emerald-700 transition">
                        {ing.name}
                      </h3>
                      {ing.category && (
                        <span className="inline-block mt-1 text-xs font-semibold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">
                          {ing.category}
                        </span>
                      )}
                    </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
          <div
            ref={modalRef}
            className="bg-white rounded-3xl shadow-2xl p-8 max-w-lg w-full mx-4 animate-scale-in"
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  onClick={closeModal}
                  disabled={submitting}
                  className="flex-1 py-3 rounded-xl bg-neutral-100 text-neutral-700 font-bold hover:bg-neutral-200 transition disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-bold hover:from-emerald-700 hover:to-emerald-800 transition shadow-lg hover:shadow-xl disabled:opacity-50"
                >
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
          <div
            ref={deleteRef}
            className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full mx-4 animate-scale-in"
          >
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 py-3 rounded-xl bg-gradient-to-r from-rose-600 to-rose-700 text-white font-bold hover:from-rose-700 hover:to-rose-800 transition shadow-lg hover:shadow-xl disabled:opacity-50"
              >
